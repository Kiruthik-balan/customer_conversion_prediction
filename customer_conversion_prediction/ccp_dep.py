...
if result:
    ...
    if res == 0:
            prediction_text = "No, The customer is highly unlikely to subscribe to the insurance."
            colored_text = f'<p style="color: red;">{prediction_text}</p>'
            st.subheader(colored_text, unsafe_allow_html=True)
    else:
        prediction_text = "yes, The customer is highly like to subscribe to the insurance."
        colored_text = f'<p style="color: green;">{prediction_text}</p>'
        st.subheader(colored_text, unsafe_allow_html=True)
